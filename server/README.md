...
5.  **Start the MongoDB Instance**:
    ...
    _To stop the service: `brew services stop mongodb-community@7.0`_

6.  **Verify MongoDB is Running**:
    ```bash
    ps aux | grep mongod
    ```
    _You should see a `mongod` process running._
...
2.  **Configure Environment Variables**:
    ...
    _Ensure `MONGODB_URI` and `PORT` are correctly set._
...
1.  **Start the Server**:
    ...
    _The server will be available at `http://localhost:3000`._
